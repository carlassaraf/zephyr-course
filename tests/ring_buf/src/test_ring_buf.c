...
ZTEST(ring_buf_init, test_reinit_clears_state)
{
	...
	rb_push(99);
	rb_init(4);
	zassert_true(rb_is_empty(), "Push discarted after rb_init");
	zassert_equal(rb_count(), 0, "Ring buffer count must be 1 after rb_init");
}
...
ZTEST(ring_buf_push_pop, test_single_push_pop)
{
	 ...
	 */
	zassert_equal(rb_push(42), 0, "Push must return 0");
	int v; 
...
ZTEST(ring_buf_push_pop, test_fifo_order)
{
	 ...
	 */
	zassert_equal(rb_push(1), 0, "Push must return 0");
	zassert_equal(rb_push(2), 0, "Push must return 0");
	zassert_equal(rb_push(3), 0, "Push must return 0");
	int v;
	zassert_equal(rb_pop(&v), 0, "Pop must return 0");
	zassert_equal(v, 1, "Value popped must be 1");
	zassert_equal(rb_pop(&v), 0, "Pop must return 0");
	zassert_equal(v, 2, "Value popped must be 2");
	zassert_equal(rb_pop(&v), 0, "Pop must return 0");
	zassert_equal(v, 3, "Value popped must be 3");
}
...
ZTEST(ring_buf_push_pop, test_push_full_returns_enospc)
{
	 ...
	 */
	rb_push(1); rb_push(2); rb_push(3); rb_push(4);
	zassert_equal(rb_push(99), -ENOSPC, "Buffer out of capacity");
}
...
ZTEST(ring_buf_boundaries, test_peek_does_not_consume)
{
	 ...
	 */
	rb_push(7); int v;
	zassert_equal(rb_peek(&v), 0, "Pop must return 0");
	zassert_equal(v, 7, "Popped value must be 7");
	zassert_equal(rb_peek(&v), 0, "Pop must return 0");
	zassert_equal(v, 7, "Popped value must be 7");
}
...
ZTEST(ring_buf_boundaries, test_pop_null_returns_einval)
{
	 ...
	 */
	zassert_equal(rb_pop(NULL), -EINVAL, "Nothing to pop");
}
